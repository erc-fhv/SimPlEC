...
import models.hyst_controller as hyst_controller
...
class HystController(mosaik_api.Simulator):
    Δ_T = 60
    def __init__(self):
        super().__init__(META)
    ...
    def init(self, sid, time_resolution, eid_prefix=None):
        if float(time_resolution) != self.Δ_T:
            raise ValueError(f'HystController only supports time_resolution={self.Δ_T}., but'
                             ' %s was set.' % time_resolution)
        if eid_prefix is not None:
            self.eid_prefix = eid_prefix
        return self.meta
    
    def create(self, num, model, T_is_0=21):
        next_eid = len(self.entities)
        entities = []

        for i in range(next_eid, next_eid + num):
            model_instance = hyst_controller.HystController(T_is_0=T_is_0)
    ...
    def get_data(self, outputs):
        ...
        for eid, attrs in outputs.items():
            ...
            data[eid] = {}
            for attr in attrs:
                if attr not in self.meta['models']['HystController']['attrs']:
...
def main():
    return mosaik_api.start_simulation(HystController())
