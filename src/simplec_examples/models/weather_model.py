...
class LocalWeather():
    ...
    def step(self, time):
        temp_air, dni, dhi, ghi = self._weather_df.loc[time,
            ['temp_air', 'dni', 'dhi', 'ghi']].values
        apparent_zenith, azimuth = pvlib.solarposition.get_solarposition(time,
            self.latitude, self.longitude).loc[time, ['apparent_zenith', 'azimuth']].values

        return {'T_air': temp_air, 'dni': dni, 'dhi': dhi, 'ghi': ghi,
